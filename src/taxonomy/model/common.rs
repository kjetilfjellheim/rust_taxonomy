...
impl ListRequest {
    pub fn new(start_index: i64, number_of_elements: i64) -> ListRequest {
        ListRequest { start_index: start_index, number_of_elements: number_of_elements }
    }
}
...
impl<T> ListResponse<T> {
    pub fn new(start_index: i64, number_of_elements: i64, queried_number_of_elements: i64, queried_elements: Vec<T>) -> ListResponse<T> {
        ...
        if queried_number_of_elements > queried_elements.len() as i64 {
            ...
            vec = queried_elements.into_iter().take(number_of_elements as usize).collect();
        }
        ListResponse {
